...
<script type="text/javascript">
	$().ready(function() {
     ...
     // Name input validation
        ...
        onSelect: function () {
            $(this).trigger('input'); // Trigger input event to handle validation
        }
    });

    // Handle input event for datepicker input
    $('#datepicker').on('input', function () {
        let dateValue = $(this).val().trim();
        let $message = $('#dateMessage'); // Success message element
        let $error = $('#dateError'); // Error message element
        ...
        if (dateValue === '') {
            ...
            // 생년월일이 선택되었을 때
            $(this).removeClass('invalid').addClass('valid'); // Add blue border
...
input {
	background-color: #eed; /* Light sky blue */;
	border: none;
	padding: 12px 15px;
	margin: 8px 0;
}
/* Style the select element similar to input */
select {
	background-color: #eed; /* Light sky blue */
	padding: 12px 15px;
	margin: 8px 0;
...
/* Option elements styling - limited customization */
option {
	background-color: #eed;
	padding: 12px 15px;
	/* Customization options for font, color, etc., are limited */
}
</style>
</head>
<body>
	...
	<div class="main" style="background-color: #EBF7FF;">
		...
		<div style="width: 60%; margin: 0 auto;">
			...
			<form action="doginsert.action" id="dogForm">
				<br /> <input type="hidden" id="filePathInput" name="filePath">
				<!-- 파일 경로를 저장할 hidden input -->
				<div class="photo-container" style="background-color: white; text-align: left; display: flex; justify-content: space-between;">
					...
					<div style="font-family: Pretendard; font-weight: 600; font-size: 14pt; width: 60%;">
						<br> <a href="dogcodeinsertform.action" id="link" style="color: gray; font-size: 12pt; font-weight: 400; float: right;">코드 입력하기&nbsp;&nbsp;&nbsp;</a>
